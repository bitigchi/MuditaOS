...
namespace utils
{
    ...
    std::vector<Language> LangLoader::getAvailableDisplayLanguages() const
    {
        ...
        /// Exclude languages from the list(if any)
        languageNames.erase(std::remove_if(languageNames.begin(),
                                           languageNames.end(),
                                           [this](const auto &lang) { return matcher(lang); }),
                            languageNames.end());
    ...
    LangLoader::LangLoader(std::vector<Language> excludeList) : excludedLanguages{std::move(excludeList)}
    {}

    bool LangLoader::matcher(const std::string_view language) const
    {
